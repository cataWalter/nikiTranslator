...
from myTranslator import new_translation, new_string_translation
...
class Ui_HCI(object):
    def setupUi(self, HCI):
        ...
        languages = ['Afrikaans', 'Arabic', 'Bangla', 'Bosnian', 'Bulgarian', 'Cantonese', 'Catalan',
                     'Chinese Simplified', 'Chinese Traditional', 'Croatian', 'Czech', 'Danish', 'Dutch', 'English',
                     'Estonian', 'Fijian', 'Filipino', 'Finnish', 'French', 'German', 'Greek',
                     'Haitian Creole', 'Hebrew', 'Hindi', 'Hmong Daw', 'Hungarian', 'Indonesian', 'Italian', 'Japanese',
                     'Kiswahili', 'Klingon', 'Klingon (plqaD)', 'Korean', 'Latvian', 'Lithuanian', 'Malagasy'
            , 'Malay', 'Maltese', 'Norwegian', 'Persian', 'Polish', 'Portuguese', 'Queretaro Otomi', 'Romanian',
                     'Russian', 'Samoan', 'Serbian (Cyrillic)', 'Serbian (Latin)', 'Slovak', 'Slovenian', 'Spanish'
            , 'Swedish', 'Tahitian', 'Tamil', 'Thai', 'Tongan', 'Turkish', 'Ukrainian', 'Urdu', 'Vietnamese', 'Welsh',
                     'Yucatec Maya']
    ...
    def printfunc(self):
        ...
        if (target == 'error'):
            ...
            x = result.decode("utf-8")
            """
            text = text.replace('\n', ' ')
            x = "ue"
            x = new_string_translation(text)
            self.textEdit_2.setText(x)
    ...
    def retranslateUi(self, HCI, data):
        ...
        self.commandLinkButton.clicked.connect(
            lambda: webbrowser.open('https://ufl.qualtrics.com/jfe/form/SV_eONVoLWuM7cjqNn'))
        self.commandLinkButton_2.setText(_translate("HCI", "Complete Interaction"))
        self.commandLinkButton_2.clicked.connect(
            lambda: webbrowser.open('https://ufl.qualtrics.com/jfe/form/SV_9uY6i1QKmNNw5O5'))
